...
class StatusRuanganView(QWidget):
    ...
    def refresh_data(self):
        """Memperbarui data ruangan dari Supabase dan merender kartu secara asinkron."""
    ...
    def _fetch_data_worker(self, filter_text, today_str, current_time):
        ...
        reservasi_data = supabase.table('reservasi').select(
            "*", f"status=eq.Disetujui&tanggal=eq.{today_str}"
        ) or []
        ...
        for r in rooms_data:
            ...
            if base_status in ("Tidak Tersedia", "Nonaktif", "Maintenance"):
                ...
                for res in room_reservations.get(rid, []):
            ...
            if filter_text != "Semua" and filter_text != status:
                continue
    ...
    def _on_data_fetched(self, result):
        ...
        if hasattr(self, 'stats_label'):
            self.stats_label.setText(
                f"{counts.get('Tersedia', 0)} TERSEDIA · "
                f"{counts.get('Terbooking', 0)} TERBOOKING · "
                f"{counts.get('Terpakai', 0)} TERPAKAI"
            )
        ...
        if hasattr(self, 'notif_text') and hasattr(self, 'notif_banner'):
            if upcoming_reservations:
                ...
                self.notif_text.setText(
                    f"INFO: Ruangan {ruang_nama} telah dipesan "
                    f"untuk '{keperluan}' pada pukul {jam}."
                )
                self.notif_banner.show()
            else:
                self.notif_banner.hide()

        # Tampilkan pesan kosong jika tidak ada ruangan
        ...
        for i, (name, status, badge_class) in enumerate(filtered_rooms):
            ...
            # DIHAPUS: is_dark = theme_manager.is_dark — tidak pernah dipakai
            ...
            # 3. Kubus 3D
            cube_color = "#22C55E"
            if status in ("Terpakai", "Digunakan", "Tidak Tersedia", "Nonaktif", "Maintenance"):
                cube_color = "#EF4444"
            elif status in ("Terbooking", "Dosen"):
                cube_color = "#F59E0B"

            cube_widget = CubeWidget(cube_color, should_animate=False)
